...
Данные разных строительных компаний живут в одной базе и не пересекаются:
у каждой прикладной строки есть `org_id`, а составные внешние ключи
`(id, org_id)` физически запрещают связать сущности разных компаний.
...
- [`new_org.ps1`](new_org.ps1) / [`new_org.sql`](new_org.sql) — завести новую компанию и её первого администратора;
- [`migrate.ps1`](migrate.ps1) — накатить недостающие миграции на работающую базу и показать отчёт;
- [`migrations/`](migrations) — сами миграции, накатываются по порядку;
- [`verify.sql`](verify.sql) — проверка изоляции: нет ли ссылок между компаниями;
- [`drift.ps1`](drift.ps1) / [`drift.sql`](drift.sql) — сверка живой базы со `schema.sql`;
- [`inspect.ps1`](inspect.ps1) / [`inspect.sql`](inspect.sql) — подробный дамп фактической структуры.

Все `.ps1` сами находят `psql.exe` и выставляют UTF-8, добавлять его в PATH не нужно.

## Расхождения базы и репозитория

Если кто-то правил базу напрямую, `schema.sql` про это не знает, и следующая
установка с нуля даст другую структуру. Проверка:

```powershell
powershell -ExecutionPolicy Bypass -File db\drift.ps1
```

Пять пустых разделов — расхождений нет. Иначе будут показаны лишние и
недостающие таблицы и колонки, а также различия в `NOT NULL`, `DEFAULT` и типах.

`drift.sql` сгенерирован из `schema.sql` и руками не правится. После любого
изменения схемы эталон пересобирается:

```powershell
pip install pglast
python db\tools\gen_drift.py
```
...
Логины и телефоны уникальны во всей базе, поэтому для второй компании берите
логин с префиксом (`elit.admin`) или номер телефона. Пароль администратора —
`0000`, приложение потребует сменить его при первом входе.

## Обновление работающей базы

```powershell
powershell -ExecutionPolicy Bypass -File db\migrate.ps1
```

Скрипт сам определяет, какие из четырёх миграций нужны, накатывает их по
порядку и в конце показывает отчёт по изоляции и сверку со `schema.sql`.

`setup.ps1` пересоздаёт базу начисто и миграций не требует.
